...
public class InsertDelegateSingleStringImpl implements IndexInserterDelegate {
	private static Logger log = Logger
			.getLogger(InsertDelegateSingleStringImpl.class);
	...
	public InsertDelegateSingleStringImpl(String _key, String _metadata,
			String[] _collection, String _fPath) {
	...
	public void insert(IndexWriter writer) throws IOException {
		...
		try {
			...
			doc = handler.getDocument(new ByteArrayInputStream(metadata
					.getBytes("UTF-8")));

			doc.add(new Field("key", key, Field.Store.YES,
					Field.Index.NOT_ANALYZED));
			for (String collection : this.collection) {
				doc.add(new Field("collection", collection, Field.Store.YES,
						Field.Index.NOT_ANALYZED));
			}
			...
			if (luceneHandler
				...
				doc.add(new Field("maceenrichedlom", exml, Field.Store.YES,
						Field.Index.NOT_ANALYZED,
