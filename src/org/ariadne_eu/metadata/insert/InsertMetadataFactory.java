...
/**
 * Created by ben Date: 5-mei-2007 Time: 19:02:22 To change this template use
...
public class InsertMetadataFactory {
		...
		for (int language = -1; language < 10; language++) {
			...
			if (implementation != null) {
				try {
					...
					InsertMetadataImpl insertMetadata = (InsertMetadataImpl) implClass
							.newInstance();
					insertMetadata.setLanguage(language);
					insertMetadata.initialize();
					cachedImplementations.add(insertMetadata);
				} catch (Exception e) {
					log.error("Error while initializing insertMetadata class",
							e);
				}
			}
		}
	...
	public static InsertMetadataImpl[] getInsertImpl() {
		return (InsertMetadataImpl[]) cachedImplementations
				.toArray(new InsertMetadataImpl[cachedImplementations.size()]);
	}

	public static void insertMetadata(String identifier, String metadata,
			String collection) throws InsertMetadataException {
