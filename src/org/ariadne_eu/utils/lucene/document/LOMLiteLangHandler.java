...
public class LOMLiteLangHandler extends DocumentHandler {

	private static Logger log = Logger.getLogger(LOMLiteLangHandler.class);
	...
	public void endElement(String uri, String localName, String qName)
		...
		// 23/10/12 Change for supporting classification related facets
		// @NaturalEurope
		 ...
		 * { if
		 * (tmpBranche.endsWith("classification.taxonpath.taxon.entry.string"))
		 * { doc.add(new Field(tmpBranche,
		 * elementBuffer.toString().trim().toLowerCase(),
		 * Field.Store.YES,Field.Index.ANALYZED));// XXX } contents =
		 * contents.concat(" " + elementBuffer.toString().toLowerCase()); }
		 */
		...
		// Catalog + entry
				...
				.matches(".*general.identifier\\.((catalog)|(entry))")) {
		...
		// Metametadata Catalog + entry
				...
				.matches(".*metametadata.identifier\\.((catalog)|(entry))")) {
			if (tmpBranche.endsWith("identifier.catalog")) {
				doc.add(new Field(tmpBranche.toLowerCase(), elementBuffer
						.toString().trim(), Field.Store.YES,
						Field.Index.NOT_ANALYZED));// XXX
		...
		// Metametadata role.source + role.value
				...
				.matches(".*metametadata.contribute.role\\.((source)|(value))")) {
		...
		/*
		 * // general.keyword.string else if
		 * (tmpBranche.matches(".*general.keyword.string")) { String format =
		 * elementBuffer.toString().toLowerCase().trim(); doc.add(new
		 * Field(tmpBranche.toLowerCase(), format, Field.Store.YES,
		 * Field.Index.ANALYZED));// XXX contents = contents.concat(" " +
		 * elementBuffer.toString().toLowerCase()); }
		 */
	...
	public static void main(String args[]) throws Exception {
		LOMLiteLangHandler handler = new LOMLiteLangHandler();
		Document doc = handler
				.getDocument(new FileInputStream(
						new File(
								"/Sandbox/temp/AriadneWS/mdstoreARIADNE/ARIADNE/BLKLKP325.xml")));
